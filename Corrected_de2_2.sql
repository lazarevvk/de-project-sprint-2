--Теперь всё стало понятно, спасибо большое, что ответили более развёрнуто!
...
with CTE as (
   select 
    ...
    MAX(s.state) as max_state,
    MIN(s.state_datetime) as min_state_datetime,
    case 
        when MAX(s.status) = 'in_progress' THEN NULL
        else MAX(s.state_datetime)
    end as max_state_datetime   
from
    shipping s
join
    (select shippingid, MAX(state_datetime) AS max_datetime
     ...
     group by shippingid) max_time
on
    s.shippingid = max_time.shippingid AND s.state_datetime = max_time.max_datetime
group by
    s.shippingid, s.status, s.state
)

insert into shipping_status (shipping_id, status, state, shipping_start_fact_datetime, shipping_end_fact_datetime)
select cte.shippingid, cte.max_status, cte.max_state, cte.min_state_datetime, cte.max_state_datetime
from cte;
